...
const UserItemsScreen = (props) => {
	...
	if (!isLoading && userItems.length === 0) {
		return <View style={styles.loading}>
			...
			<DefaultText>Tap the + on the top right corner to start adding some.</DefaultText>
		</View>
	}
    ...
    return (
       <FlatList onRefresh={loadItems} refreshing={isRefreshing} data={userItems} renderItem={(itemData) => (
        <ProductItem
